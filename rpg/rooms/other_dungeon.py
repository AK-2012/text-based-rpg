...
def other_dungeon(self):
    if "entered_other_dungeon" not in self.flags:
        print(
        ...
        while True:
            do = (
                input(
                    f"{Fore.BLUE}{Style.BRIGHT}What would you like to do? {Style.RESET_ALL}"
                )
                .strip()
                .lower()
            )
        ...
        # player has key, first entrance to other dungeon
        if "staff" not in self.items:
            ...
            print(
                "You tell him you were another prisoner, but escaped and came here to see what was here. "
            ...
            print(
                "As you thank him and turn to leave back to the colosseum, the old man speaks for the"
                ' first time. "We will pray for you. Good luck." You thank them again and head back to '
                "the colosseum. "
            )
            self.flags.add("entered_other_dungeon")
            return "colosseum"
    # player has entered other dungeon before, heading back
    print(
    ...
    print(
        '"You really should go. We will do fine here alone. You have your own path to take. Although,'
