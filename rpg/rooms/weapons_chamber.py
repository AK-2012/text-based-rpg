...
def weapons_chamber(self):
    ...
    take = (
        input(
            f"{Fore.BLUE}{Style.BRIGHT}Which weapon would you like to take? {Style.RESET_ALL}"
        )
        .strip()
        .lower()
    )
    ...
    self.damage_check()
    print(
        f"You took the {take}. You deal {self.attack} damage now. You return to the witch's tower."
    )
    return "witch_tower"
