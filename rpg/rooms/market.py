...
def market(self):
    ...
    while True:
        go = (
            input(
                f"{Fore.BLUE}{Style.BRIGHT}\nWhere would you like to go? {Style.RESET_ALL}"
            )
            .strip()
            .lower()
        )
    ...
    if "has_daughter_quest" not in self.flags:
        ...
        print(
            "\"Am I okay! Of course not! I've been handing these flyers out all morning, and nobody's batted an eye. "
        ...
        while help not in ["n", "y", "no", "yes"]:
            help = (
                input(f'{Fore.BLUE}{Style.BRIGHT}"Hmm?" {Style.RESET_ALL}')
                .strip()
                .lower()
            )
        # not helping
