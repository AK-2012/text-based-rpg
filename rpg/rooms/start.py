...
def start(self):
    # beginning of game.
    print(
        "You are in a dungeon. You remember you are a ruler of your people raising an uprising against the malevolent king. "
    ...
    # stats
    print(
        f"{Fore.GREEN}{Style.BRIGHT}Current stats: {self.hearts} hearts, {self.attack} attack damage. You have no items.\n"
    )
    while True:
        do = (
            input(
                f"{Fore.BLUE}{Style.BRIGHT}What would you like to do? (leave) (take ____) {Style.RESET_ALL}"
            )
            .strip()
            .lower()
        )
        if "take" in do and "hand" in do and "hand" not in self.items:
            ...
            while True:
                ...
                if "take" in do and "sword" in do and "sword" not in self.items:
                    ...
                    self.damage_check()
                    print(
                        f"{Fore.YELLOW}{Style.BRIGHT}Sword added to inventory. You can deal {self.attack} attack damage. \n"
                    )
                elif do == "leave":
